...
class Flow_worker_class_garph(QObject):
    ...
    def paint_grah(self):
        ...
        if tdelta.total_seconds() > time_gap:
            ...
            while 1:
                ...
                time.sleep(0.1)
                tdelta = self.time_2 - self.time_1
                if tdelta.total_seconds() > 0.4:
                    self.reload_signal_1.emit()  # Перезапуск расходомера
                    time.sleep(0.1)
                if tdelta.total_seconds() > 0.5:
                    break
            ...
            if self.y_buff_1 is not None:
                ...
                self.gas_consumption_m1 = self.gas_consumption_m1 + self.y_main_plot_m1[
                    -1] / 3600 * time_gap  # Расход газа
                self.gas_consumption_signal_1.emit(self.gas_consumption_m1)
            ...
            if self.y_buff_2 is not None:
                ...
                problem = 1  # Проблема, поставить красные точки если Nan
                self.y_main_plot_m2.append(self.y_main_plot_m1[-1])  # В конец масива
                self.gas_consumption_m2 = self.gas_consumption_m2 + self.y_main_plot_m1[-1] / 3600 * time_gap  # Расход
                self.gas_consumption_signal_2.emit(self.gas_consumption_m2)
    ...
    def Write_data_first(self):
        ...
        my_file.write(f"Time\t" 
                f"value_on_device_{self.name_1}\tvalue_in_program_{self.name_1}\t" +
                f"Gas_consumption_{self.name_1}\tTemperature_{self.name_1}\t" +
                f"value_on_device_{self.name_2}\tvalue_in_program_{self.name_2}\t" +
                f"Gas_consumption_{self.name_2}\tTemperature_{self.name_2}\n")
        my_file.close()
